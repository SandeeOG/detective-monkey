...
- **Backend:** FastAPI + SQLAlchemy + SQLite — REST API under `/api`.
  ...
  - AI coach (`coach.py`): uses Claude if `ANTHROPIC_API_KEY` is set, otherwise a deterministic context-aware fallback.
...
### Optional: live AI coach

By default the coach gives helpful deterministic answers. To use Claude:
...
Model defaults to `claude-sonnet-4-6` (override with `DM_LLM_MODEL`).
